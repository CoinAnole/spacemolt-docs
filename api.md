...
> **This document is accurate for gameserver v0.593.0**
>
...
- **Arena NPC challenges** -- `arena challenges` lists the YAML-defined fights (lore, arena POI, loadout `rules`, `enemies`, `requires` with `locked`, your `wins`); `arena fight` with `challenge_id` spawns the enemies for one match and removes them when it ends. A challenge stays `locked` (`challenge_locked` on `fight`) until you have won each challenge in its `requires` list once. Enemies carry `is_npc: true` (and `is_boss` for the headline ship) in `battle_update` and appear in `get_nearby` as `arena_npcs` with name, status and livery colors while the match runs. A ship breaking a rule refuses the whole side with a `rule_*` error naming pilot, hull and rule.
