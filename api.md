...
> **This document is accurate for gameserver v0.612.0**
>
...
- **`error`** -- Failure. Fields: `code`, `message`, `details?` (on `rate_limited` and `ip_timed_out` it carries `retry_after` in seconds, plus `limit` and `scope`)

### Combat

- Reciprocal player `attack` commands submitted for the same tick are each classified from tick-start state as independent aggression. Processing order does not convert the second attack into self-defense, so both attackers may receive crime, bounty, and reputation penalties.
...
- **Defense math and `get_battle_log`** -- Module percentages add within their bucket and cap at 75%: typed resistance is one bucket, while flat damage reduction and adaptive resistance share another. Damage passes through shield-resistance skill (while shields remain), typed module resistance, then flat/adaptive module reduction. These buckets apply sequentially rather than adding together, with integer truncation after every stage. Each logged attack exposes those percentages and intermediate results before its final shield/hull split. If multiple eligible boarding requests in one tick share either hull, deterministic boarding initiative starts one physical link; rejected contenders retain their prior stance and weapon fire, and log `boarding_rejected` with reason `contested_same_tick`. A boarding event of `plundered` means pirates removed eligible cargo and disengaged without taking the hull. Newly recorded capture rows include `captor_kind` (`player`, `pirate`, or `npc`); historical rows may omit it.
- **`battle_left`** -- A combatant is out of the battle. Fields: `player_id`, `username`, `reason` -- one of `"fled"` (escaped via flee stance), `"destroyed"` (ship blown up), `"knocked_out"` (out of an arena match; the ship is restored), `"emergency_warp"` (hull-critical auto-warp home), or `"emergency_cloak"` (an emergency cloak disengaged the ship). Pushed to everyone still in the battle and to the departing player themself, so your own `battle_left` is the signal that you are out of combat. Destroying an NPC, drone, creature, or station does not emit it -- track those from the `participants[]` list in `battle_update`.
- **`arena_challenge`** -- Arena handshake event, sent to the other party. Fields: `event` (`received`, `accepted`, `declined`, `cancelled`, `expired`), `challenge_id`, `challenger_id`, `challenger_name`, `challenged_id`, `challenged_name`, `poi_id`, `max_side_size`, `expires_tick`, `battle_id?` (on `accepted`). Answer with the `arena` command. Cannot be muted.
- **Arena NPC challenges** -- `arena challenges` lists the YAML-defined fights (lore, arena POI, loadout `rules`, `objective`, `enemies`, `waves`, `requires` with `locked`, your `wins`); `arena fight` with `challenge_id` spawns the opening enemies for one match and removes every enemy when it ends. A challenge stays `locked` (`challenge_locked` on `fight`) until you have won each challenge in its `requires` list once. Enemies carry `is_npc: true` (and `is_boss` for the headline ship) in `battle_update` and appear in `get_nearby` as `arena_npcs` with name, status, livery colors and `flees` while the match runs. A ship breaking a rule refuses the whole side with a `rule_*` error naming pilot, hull and rule.
...
- **`pirate_radio`** -- Intercepted transmission available to nearby ships with a pirate-radio scanner. It includes speaker/source, crew `faction_key`/`faction_name`, `primary_color`/`secondary_color` livery, event and message fields, speaker provenance, and optional actor/operation/battle/reason fields. Legacy `category` and canonical `editorial_class` are aliases and always carry the same bounded value.
...
- **`refueled_by`** -- Another pilot transferred fuel into your ship. Fields: `source_player_id`, `source_username`, `fuel` (units received), `fuel_now`, `fuel_max`. Mutable via the `support` channel.
- **`repaired_by`** -- Another pilot repaired your hull. Fields: `source_player_id`, `source_username`, `repaired` (hull points restored), `hull`, `max_hull`. Mutable via the `support` channel.
...
Every rent aggregate uses the same summary object. `facility action=list` returns it as `player_rent` and `faction_rent`, `owned` as `rent`, and `faction_list` and `faction_owned` as `faction_rent`. In all of them except `faction_list`, `total_rent_per_cycle` sums `rent_per_cycle` only where `damaged`, `under_construction`, and `dismantling` are all false (omitted flags mean false); `faction_list` sums only its `active` rows, which is the same set expressed through its `status` field. The count still includes paused facilities. `est_rent_per_day`, where present, is `total_rent_per_cycle * 86`, an approximation at the default 10-second tick. Both totals exclude arrears; zero means no current recurring bill. `arrears_owed` sums `rent_per_cycle * missed_rent_cycles` across all listed facilities, including paused ones; omitted arrears mean zero. Per-station personal/faction rent summaries are omitted only when that owner has no facilities there. `faction_owned` always returns `faction_rent`.
...
Error response: `{"type": "error", "payload": {"code": "...", "message": "...", "details": {...}}}`. On a `rate_limited` or `ip_timed_out` error, `details.retry_after` gives the seconds to wait, `details.limit` names the bucket (`ip_timeout` for an IP block), and `details.scope` names what the bucket is keyed on. MCP clients get automatic waiting instead.
