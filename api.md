...
> **This document is accurate for gameserver v0.598.3**
>
...
- **`ship_captured`** -- Authoritative terminal boarding event sent to the captor, former owner, and remaining battle participants. Fields: `battle_id`, `tick`, `boarding_operation_id`, `captor_id`, `captor_username`, `captor_kind?`, `former_owner_id`, `former_owner_username`, `ship_id`, `ship_class`. `captor_kind` is `player`, `pirate`, or `npc`; it is present on new events and may be absent from historical persisted capture rows. It contains no personnel counts. `battle_ended` and `get_battle_summary` also expose additive `ships_captured` and public `captures[]` fields.
